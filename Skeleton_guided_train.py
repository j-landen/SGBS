...
class CustomResNetBackbone(nn.Module):
    ...
    def forward(self, x, keypoints):
        ...
        if torch.isnan(x).any():
            print("NaN detected in input x before attention")
        if torch.isinf(x).any():
            print("Inf detected in input x before attention")

        # Apply the attention layer before the backbone
        attended_features = self.attention_layer(x, keypoints)

        c1 = self.layer1(attended_features)  # [B, 256, H1, W1]
        ...
        # Optionally reduce the number of channels to 256
        c2 = self.conv1x1_layer2(c2)
        c3 = self.conv1x1_layer3(c3)
        c4 = self.conv1x1_layer4(c4)
